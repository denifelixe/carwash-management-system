...
import { memberPortalUrls, urlPath } from '@/composables/useMemberPortalRoutes';
import type {
...
const urls = computed(() => memberPortalUrls(page.props.mode));

const tabs = computed<
    { key: string; label: string; icon: LucideIcon; url: string }[]
>(() => [
    {
        key: 'dashboard',
        label: 'Beranda',
        icon: House,
        url: urls.value.dashboard,
    },
    {
        key: 'stamps',
        label: 'Riwayat',
    ...
    { key: 'profile', label: 'Profil', icon: User, url: urls.value.profile },
]);

const currentPath = computed<string>(() => page.url.split('?')[0]);

function isCurrentTab(url: string): boolean {
    return currentPath.value === urlPath(url);
}
