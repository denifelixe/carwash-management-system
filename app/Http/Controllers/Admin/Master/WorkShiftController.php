...
class WorkShiftController extends Controller
{
    public function index(Request $request, AdminShell $adminShell): Response
    {
        ...
        $workShifts = AdminWorkShift::query()
            ->withCount('admins')
            ->orderBy('starts_at')
            ->orderBy('name')
            ->get();
        ...
        return Inertia::render('admin/master/WorkShifts', [
            ...
            'workShifts' => $workShifts->map(fn (AdminWorkShift $workShift): array => $this->workShiftData($workShift))->all(),
            'capabilities' => [
    ...
    {
        AdminWorkShift::query()->create($request->validated());

        return to_route('admin.master.work-shifts.index')->with('success', 'Shift berhasil ditambahkan.');
    }

    public function update(UpdateWorkShiftRequest $request, AdminWorkShift $workShift): RedirectResponse
    {
    ...
    public function destroy(AdminWorkShift $workShift): RedirectResponse
    {
    ...
    private function workShiftData(AdminWorkShift $workShift): array
    {
        return [
            'id' => $workShift->id,
            'key' => $workShift->key,
